...
class ActionEditor:
    ...
    left_timing_box: widgets.VBox = field(init=False)
    left_start_input: widgets.IntText = field(init=False)
    left_end_input: widgets.IntText = field(init=False)
    ...
    def __post_init__(self) -> None:
        ...
        self.left_timing_box = self._build_left_timing_box()
        ...
        row2 = widgets.HBox(
            [
                ...
                widgets.VBox([widgets.HTML("<b>Tool</b>"), self.tool_dd]),
    ...
    def _initial_camera_segments(self) -> List[dict]:
        ...
        if isinstance(existing, list) and existing:
            ...
            while len(rows) < 3:
                rows.append({"action_code": "(not set)", "start_frame": 0.0, "end_frame": 0.0})
            return rows
        ...
        while len(rows) < 3:
            rows.append({"action_code": "(not set)", "start_frame": 0, "end_frame": 0})
        return rows[:3]

    def _build_camera_actions_box(self) -> widgets.VBox:
        rows = []
        self.camera_action_widgets = []
        for idx, seg in enumerate(self._initial_camera_segments(), start=1):
            action_dd = _dropdown(CAMERA_ACTION_CODES + ["(not set)"], seg.get("action_code", "(not set)"), width="220px")
            start_input = widgets.IntText(
                value=int(seg.get("start_frame", 0) or 0),
                layout=widgets.Layout(width="140px"),
            )
            end_input = widgets.IntText(
                value=int(seg.get("end_frame", 0) or 0),
                layout=widgets.Layout(width="140px"),
            )
            self.camera_action_widgets.append(
                {"action_dd": action_dd, "start_input": start_input, "end_input": end_input}
            )
            row = widgets.HBox(
                [
        ...
        box = widgets.VBox(
            [
                widgets.HTML(
                    ...
                    "<div style='font-size:12px;color:#475569;'>Up to 3 camera actions, each with its own frame span.</div>"
                ),
                *rows,
            ]
        )
    ...
    def _build_left_timing_box(self) -> widgets.VBox:
        default_start = int(
            _safe_float(
        ...
        self.left_start_input = widgets.IntText(
            value=default_start,
            layout=widgets.Layout(width="140px"),
        )
        self.left_end_input = widgets.IntText(
            value=default_end,
            layout=widgets.Layout(width="140px"),
        )
        return widgets.VBox(
            [
                widgets.HTML(
                    "<div style='font-weight:700;color:#0f172a;margin-top:4px;'>Left instrument timing</div>"
                    "<div style='font-size:12px;color:#475569;'>Annotate the frame span for this left-instrument action.</div>"
                ),
                widgets.HBox(
                    [
                        widgets.HTML("<b>Start</b>"),
                        self.left_start_input,
                        widgets.HTML("<b>End</b>"),
                        self.left_end_input,
                    ]
                ),
    ...
    def _update_camera_mode(self) -> None:
        ...
        self.single_action_box.layout.display = "none" if is_camera else ""
        self.camera_actions_box.layout.display = "" if is_camera else "none"
        self.left_timing_box.layout.display = "" if is_left else "none"
    ...
    def serialize(self, rank: int) -> Optional[dict]:
        ...
        if self.actor_dd.value == "left_instrument":
            out["left_action_start_frame"] = int(self.left_start_input.value or 0)
            out["left_action_end_frame"] = int(self.left_end_input.value or 0)
        else:
...
class AuditVersionInterface:
    ...
    def launch(self) -> None:
        ...
        display(widgets.VBox([self.header_html, controls, self.status_html, self.main_box]))
